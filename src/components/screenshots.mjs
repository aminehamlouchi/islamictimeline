...
const MIME = {
  ...
  ".svg": "image/svg+xml", ".woff2": "font/woff2", ".png": "image/png", ".txt": "text/plain",
};
...
async function shot(name, { url = "/", w = 1440, h = 900, dark = false, onboard = false, hover = null, click = null, waitMs = 700 }) {
  ...
  if (!onboard) await ctx.addInitScript(() => localStorage.setItem("itl-onboarded", "1"));
  ...
  console.log(`${name}: markers=${counts.markers} line=${counts.line} :: ${counts.title}`);
  await ctx.close();
}
...
await shot("06-compare", { url: "/?cmp=al-bukhari,ibn-taymiyya,al-nawawi&cv=1&y=1100&z=0.8" });
...
await shot("11-mobile-selected", { url: "/?sel=al-nawawi", w: 390, h: 844, waitMs: 1100 });
